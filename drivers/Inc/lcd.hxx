...
namespace LCD
{
    ...
    const char SET_CGRAM_INSTRUCTION_BIT = 1 << 6;

    // Sets DDRAM address in address counter.
    const char SET_DDRAM_INSTRUCTION_BIT = 1 << 7;
    ...
    public:
        ...
        {
            uint8_t ccgram_cmd = SET_CGRAM_INSTRUCTION_BIT | address;
            command(ccgram_cmd);
            wait_us(40);
        }
        ...
        {
            uint8_t ddram_cmd = SET_DDRAM_INSTRUCTION_BIT | address;
            command(ddram_cmd);
            wait_us(40);
        }

        void print(const char *string, uint8_t start_position = 0x0)
        {
            set_ddram_address(start_position);
            while (*string)
            {
                write(*string++);
                wait_us(40);
